...
var View = module.exports = new Class({
	...
	_setupClassNamesBindings: function _setupClassNamesBindings() {
		...
		classNames.forEach(function(name) {
			function computedClass(newClass, oldClass) {
				var el = this.get('element');
				if (!el) {
					return;
				}
				if (oldClass) {
					el.removeClass(oldClass === true ? name._keys[0] : oldClass);
				}
				if (newClass) {
					el.addClass(newClass === true ? name._keys[0] : newClass);
				}
			}
			if (type.isFunction(name) && name.isProperty) {
				...
				name.toString = function() {
					...
					if (currVal) {
						return currVal === true ? name._keys[0] : currVal;
					} else {
						return '';
					}
				};
