    ...
    _onContentChange: function _onContentChange(newVal, oldVal) {
        ...
        if (newVal) {
            this[contentObsKey] = newVal.observe('array', function() {
                view._onArrayChange.apply(view, [this].concat(SLICE.call(arguments)));
            }, observeArrayDeeply);
        }

		view._onArrayChange(newVal, 0, oldVal || [], newVal || []);
    },

    _onArrayChange: function _onArrayChange(array, index, removed, added) {
        // 3 paths
		...
		if (added.length) {
			for (var a = 0; a < added.length; a++) {
				var v = this.createChildView(array[a]);
				addedViews.push(v);
			}
		}
