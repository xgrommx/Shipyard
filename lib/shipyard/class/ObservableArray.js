...
var ObservableArray = module.exports = new Class({
	...
	// Listen to changes so we can fire an ArrayChange event
	onPropertyChange: function arrayChangeObs(prop, newVal, oldVal) {
		var index = parseInt(prop, 10);
		if (!isNaN(index)) {
			this.emit('propertyChange', 'array', index, newVal, oldVal);
		}
	}
...
ObservableArray.implement({
	...
	splice: function splice(index, howMany /*, args...*/) {
		...
		var highestLength = Math.max(this.length, length);
		for (var i = 0; i < highestLength; i++) {
			if (oldCopy[i] !== this[i]) {
				this.emit('propertyChange', i, this[i], oldCopy[i]);
			}
		}
